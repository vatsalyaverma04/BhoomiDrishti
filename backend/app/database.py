...
class LandRecordsDatabase:
    ...
    def _load_or_seed(self):
        candidate_file = RECORDS_FILE
        if not candidate_file.exists():
            bundled_file = BASE_DIR / "data" / "land_records.json"
            if bundled_file.exists():
                candidate_file = bundled_file
    ...
    def _save(self):
        try:
            RECORDS_FILE.parent.mkdir(exist_ok=True, parents=True)
            with open(RECORDS_FILE, "w", encoding="utf-8") as f:
                json.dump(self.records, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"Notice: local saving records deferred: {e}")
